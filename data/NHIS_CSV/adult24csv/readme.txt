...
SAS
In SAS, add the option ‘guessingrows’ to the import statement. The 2024 Sample Adult has 32,629 
observations and the example below specifies to use 5,000 observations to determine the length:

proc import datafile="adult24.csv" out=adult dbms=csv; getnames=yes; guessingrows=5000; run;
...
the entire 2024 Sample Adult CSV file and does not convert text strings into factors but leaves them as 
text strings. 
...
convert strings to factors. If using read_csv, add the argument ‘guess_max.’ The 2024 Sample Adult file has 
32,629 observations and the example below specifies to use 32,629 observations to determine the data type.

adult2024 <- read_csv(“adult24.csv”, guess_max = 32629)
